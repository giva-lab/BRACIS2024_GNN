...
def make_sparse_tensor(adj,tensor_type,torch_size):
    ...
    if tensor_type == 'float':
        test = torch.sparse_coo_tensor(adj['idx'].t(),
                                      adj['vals'].type(torch.float),
                                      tensor_size) ## MUDEI: estava sparse.FloatTensor antes e dando warning no Euler
        return torch.sparse_coo_tensor(adj['idx'].t(),
                                      adj['vals'].type(torch.float),
                                      tensor_size) ## MUDEI: estava sparse.FloatTensor antes e dando warning no Euler
