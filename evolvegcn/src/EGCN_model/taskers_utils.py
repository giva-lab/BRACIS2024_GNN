...
def get_sp_adj(edges,time,weighted,time_window):
    ...
    out = torch.sparse_coo_tensor(idx.t(),vals).coalesce() ## MUDEI: estava sparse.FloatTensor antes e dando warning no Euler
...
def normalize_adj(adj,num_nodes):
    ...
    sp_tensor = torch.sparse_coo_tensor(idx.t(),vals.type(torch.float),torch.Size([num_nodes,num_nodes])) ## MUDEI: estava sparse.FloatTensor 
...
def make_sparse_eye(size):
    ...
    eye = torch.sparse_coo_tensor(eye_idx,vals,torch.Size([size,size])) ## MUDEI: estava sparse.FloatTensor 
    return eye
